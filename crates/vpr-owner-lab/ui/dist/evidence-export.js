const parseIdentity = (bytes) => {
    ...
    if (!Number.isSafeInteger(value.session_sequence)
...
export const downloadSessionEvidence = async () => {
    ...
    if (!response.ok)
        throw new Error(errorCode(bytes, response.status));
