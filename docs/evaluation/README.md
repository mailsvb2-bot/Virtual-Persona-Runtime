...
`owner-conversation.json`, `visitor-conversation.json`, `quality.json` and `cost.json`, refuses to overwrite
...
session-backed QualityContract latency distributions, plus monotonic measured session duration and runtime-observed estimated/provider-charge cost.

This command is intentionally non-promoting. It does not infer acceptance, privacy/permission,
Golden or human-review results because those require separate real observations or review.
