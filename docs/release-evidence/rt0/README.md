...
Owner Lab emits sanitized `rt0-owner-lab-session-evidence-0.2` snapshots using the shared contract in `vpr-evaluation`. `vpr-rt0-session-aggregate` can combine one or more of those snapshots into deterministic latency/cost evidence without re-reading transcript, reply, raw PCM, SDP, or provider session identifiers.

The aggregate can support browser-observed first-audio, interruption-stop, first-rendered-video, reconnect, server-stage latency, complete cost totals and runtime-backed canonical playback when those fields are actually present. Schema `0.2` derives `canonical_playback_proven` from per-attempt turn/output bindings plus runtime-confirmed playback; a forged session-level boolean is rejected. `av_sync_proven` remains false. Bound aggregate receipts are exact-candidate/provider-state artifacts, but the current exit checker intentionally rejects playback-promoting aggregates until it can recompute them from the raw session snapshots itself. Treat this as evidence preparation, not RT0 completion.
