...
The aggregate can support browser-observed first-audio, interruption-stop, first-rendered-video, reconnect, server-stage latency, complete cost totals and runtime-backed canonical playback when those fields are actually present. Schema `0.2` derives `canonical_playback_proven` from per-attempt turn/output bindings plus runtime-confirmed playback; a forged session-level boolean is rejected. The exit checker now requires the raw sanitized session snapshots, recomputes their exact candidate/provider-state binding, and accepts playback-promoting aggregates only when the recomputed structure exactly matches the archived bound aggregate. `av_sync_proven` remains false and A/V sync still requires separate real evidence. This closes the playback evidence-integrity gap; it does not complete RT0.
