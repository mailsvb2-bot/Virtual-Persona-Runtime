...
The input must be raw PCM S16LE, mono, 16 kHz and must live outside the Git worktree. STT, LLM and TTS execute only through canonical `ActiveTurn::execute_stt` / `execute_llm` / `execute_tts`; realtime avatar open/close executes through `OwnerLabEngine`. The transcript is not forwarded to the LLM: the LLM reachability probe uses a fixed safe Russian prompt, and the TTS reachability probe synthesizes a fixed safe Russian phrase. TTS configuration is built by the canonical Owner Lab provider-composition layer from `VPR_OWNER_LAB_TTS_PROVIDER`, `VPR_OWNER_LAB_TTS_ENDPOINT`, `VPR_OWNER_LAB_TTS_API_KEY`, `VPR_OWNER_LAB_TTS_MODEL` and `VPR_OWNER_LAB_TTS_VOICE`; supported adapters are OpenAI Speech (`openai` / `openai-speech`) and ElevenLabs (`elevenlabs`).

The serialized probe receipt schema is `rt0-live-provider-probe-0.2`. It contains only stage latency, usage/cost counters, transcript/output character counts, exact candidate/provider-state binding, SHA-256 + duration binding for the private STT PCM input, and sanitized TTS provider/model-or-voice descriptor fingerprint plus synthesized-PCM SHA-256/duration and usage. It never stores raw input or synthesized audio, transcript text, generated reply, WebRTC signaling, provider session identifiers, or credentials. The TTS evidence is an independent credentialed reachability proof inside the probe artifact; it does **not** add TTS to or otherwise rewrite the STT + LLM + realtime-avatar provider state used by owner/visitor conversation evidence.
