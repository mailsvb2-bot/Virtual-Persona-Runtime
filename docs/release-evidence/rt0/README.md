...
## Evidence workspace preparation

Before any credentialed or browser evidence capture, prepare an external workspace from the exact clean candidate:

```text
cargo run -p vpr-evaluation --bin vpr-rt0-evidence-prepare -- /secure/evidence/rt0
```

The command derives the candidate from `git rev-parse HEAD`, rejects a dirty worktree, rejects relative or in-worktree evidence paths, verifies that the ReleaseSpec embedded in the executable exactly matches the checked-out ReleaseSpec, and writes only the exact `release-spec.md` bytes. It is idempotent when those bytes already match and fails closed rather than overwriting a conflicting ReleaseSpec.

Its JSON report lists present and missing canonical evidence artifacts and always carries `release_ready_claimed=false`. It does **not** create placeholders or synthetic defaults for provider state/probes, owner or visitor sessions, Golden observations, quality/cost, privacy/permissions, human evaluation, known limitations, or exit evidence. Those must come from their real capture/review workflows. After each evidence-capture stage, `vpr-rt0-evidence-inventory` remains the canonical completeness/binding checker.
